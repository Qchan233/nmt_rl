...
def build_RL_model(model_opt, opt, fields, checkpoint):
    print('Building model...')
    model = onmt.ModelConstructor.make_RL_model(model_opt, fields,
                                                  use_gpu(opt), checkpoint)
    if len(opt.gpuid) > 1:
        print('Multi gpu training: ', opt.gpuid)
        model = nn.DataParallel(model, device_ids=opt.gpuid, dim=1)
...
def build_RL_optim(model, checkpoint):
    saved_optimizer_state_dict1 = None
    saved_optimizer_state_dict2 = None

    if opt.train_from:
        print('Loading optimizer from checkpoint.')
        optim1 = checkpoint['optim1']
        optim2 = checkpoint['optim2']
        # We need to save a copy of optim.optimizer.state_dict() for setting
        # the, optimizer state later on in Stage 2 in this method, since
        # the method optim.set_parameters(model.parameters()) will overwrite
        # optim.optimizer, and with ith the values stored in
        # optim.optimizer.state_dict()
        saved_optimizer_state_dict1 = optim1.optimizer.state_dict()
        saved_optimizer_state_dict2 = optim2.optimizer.state_dict()

    else:
        print('Making optimizer for training.')
        optim1 = onmt.Optim(
            opt.optim, opt.lr_actor, opt.max_grad_norm,
            lr_decay=opt.learning_rate_decay,
            start_decay_at=opt.start_decay_at,
            beta1=opt.adam_beta1,
            beta2=opt.adam_beta2,
            ...
            model_size=opt.rnn_size)
        optim2 = onmt.Optim(
            opt.optim, opt.lr_critic, opt.max_grad_norm,
            lr_decay=opt.learning_rate_decay,
            start_decay_at=opt.start_decay_at,
            beta1=opt.adam_beta1,
            beta2=opt.adam_beta2,
    ...
    # essentially it builds a new optimizer with empty optimizer state and
    # parameters from the model.
    optim1.set_parameters(model.named_parameters()) # actor parameters
    optim2.set_parameters(model.named_parameters()) # critic parameters

    print(
        "Stage 1: Keys after executing optim.set_parameters" +
        "(model.parameters())")
    show_optimizer_state(optim1)
    show_optimizer_state(optim2)
    ...
    if opt.train_from:
        ...
        # this purpose.
        # See also: https://github.com/pytorch/pytorch/issues/2830
        optim1.optimizer.load_state_dict(saved_optimizer_state_dict1)
        optim2.optimizer.load_state_dict(saved_optimizer_state_dict2)
        # Convert back the state values to cuda type if applicable
        if use_gpu(opt):
            for state in optim1.optimizer.state.values():
                for k, v in state.items():
                    if torch.is_tensor(v):
                        state[k] = v.cuda()
        if use_gpu(opt):
            for state in optim2.optimizer.state.values():
                for k, v in state.items():
                    if torch.is_tensor(v):
                        state[k] = v.cuda()
        ...
        # for Adam, which saves "exp_avg" and "exp_avg_sq" state
        # (Exponential moving average of gradient and squared gradient values)
        if (optim1.method == 'adam') and (len(optim1.optimizer.state) < 1):
            raise RuntimeError(
                "Error: loaded Adam optimizer from existing model" +
                " but optimizer state is empty")
        if (optim2.method == 'adam') and (len(optim2.optimizer.state) < 1):
            raise RuntimeError(
                "Error: loaded Adam optimizer from existing model" +
                " but optimizer state is empty")
