...
class DDPGLossComputeCritic(nn.Module):

    """

    Standard NMT Loss Computation for Critic

    """


    def __init__(self, ys, values_fit, values_optim, normalization="sents"):
        super(DDPGLossComputeCritic, self).__init__()
    ...
    def _compute_loss(self):
        loss = self.criterion(self.values_fit, self.ys)
        return loss
      
class DDPGLossComputeActor(nn.Module):

    """

    Standard NMT Loss Computation for actor

    """

    def __init__(self,  ys, values_fit, values_optim, normalization="sents",):
        super(DDPGLossComputeActor, self).__init__()
        self.values_optim = values_optim
        self.values_fit = values_fit
        
    def _compute_loss(self):
        loss = -torch.mean(self.values_optim)
        return loss
